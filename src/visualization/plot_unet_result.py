...
for roads_path in ["gis/gis_osm_roads_free_1.shp", "data/gis_osm_roads_free_1.shp"]:
    try:
        ...
        if not roads_gdf.empty:
            ...
            has_roads = True
            print(
                f"Roads loaded from {roads_path}: {len(roads_gdf)} features "
...
try:
    ctx.add_basemap(
        ax_map, source=ctx.providers.CartoDB.Positron, zoom=13, alpha=0.45, zorder=0
    )
    print("Basemap drawn")
except Exception as e:
...
if has_impact:
    impact_gdf.plot(ax=ax_map, color="#8b0000", alpha=0.18, edgecolor="none", zorder=3)
    ax_map.set_xlim(*xl)
    ax_map.set_ylim(*yl)
...
if has_roads and not roads_minor.empty:
    roads_minor.plot(ax=ax_map, color="#444444", linewidth=0.5, alpha=0.45, zorder=4)
    ax_map.set_xlim(*xl)
    ax_map.set_ylim(*yl)

# --- Layer 5: Major roads (bold, like CNN) ---
if has_roads and not roads_major.empty:
    roads_major.plot(ax=ax_map, color="#111111", linewidth=2.0, alpha=0.85, zorder=5)
    ax_map.set_xlim(*xl)
    ax_map.set_ylim(*yl)
    print("Roads drawn")
...
if has_drainage:
    drainage_gdf.plot(ax=ax_map, color="#0066cc", linewidth=1.8, alpha=0.95, zorder=6)
    ax_map.set_xlim(*xl)
    ax_map.set_ylim(*yl)
...
# ── Map cosmetics ──
ax_map.set_title(
    "Vijayawada Urban Flood Risk Map  (UNet Prediction)",
    fontsize=15,
    fontweight="bold",
    pad=12,
)
...
# ============================================================
fig.suptitle(
    "VIJAYAWADA URBAN FLOOD RISK & DRAINAGE ANALYSIS DASHBOARD",
    fontsize=17,
    fontweight="bold",
    y=0.975,
)
...
print("=" * 80)
print(
    f"  Image : {flood_data.shape[0]} x {flood_data.shape[1]} px  |  Total: {total:,}"
)
